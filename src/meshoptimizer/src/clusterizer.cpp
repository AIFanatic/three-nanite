...
// Assume DEBUG_MODE is a compile-time or runtime switch
...
static void buildTriangleAdjacency(TriangleAdjacency2& adjacency, const unsigned int* indices, size_t index_count, size_t vertex_count, meshopt_Allocator& allocator)
{
	...
	DEBUG("adjacency.counts: %i\n", index_count);
	DEBUG("adjacency.offsets: %i\n", vertex_count);
	DEBUG("adjacency.data: %i\n", face_count * 3);
...
static bool appendMeshlet(meshopt_Meshlet& meshlet, unsigned int a, unsigned int b, unsigned int c, unsigned char* used, meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices, unsigned char* meshlet_triangles, size_t meshlet_offset, size_t max_vertices, size_t max_triangles)
{
	...
	{
		DEBUG("cpp if (meshlet.vertex_count + used_extra > max_vertices || meshlet.triangle_count >= max_triangles) {\n");
		DEBUG("\tcpp meshlet_offset: %i\n", meshlet_offset);
		DEBUG("\tmeshlet.triangle_offset: %i\n", meshlet.triangle_offset);
		DEBUG("\tmeshlet.triangle_count: %i\n", meshlet.triangle_count);
		DEBUG("\tmeshlet.vertex_offset: %i\n", meshlet.vertex_offset);
		DEBUG("\tmeshlet.vertex_count: %i\n", meshlet.vertex_count);
...
static unsigned int getNeighborTriangle(const meshopt_Meshlet& meshlet, const Cone* meshlet_cone, unsigned int* meshlet_vertices, const unsigned int* indices, const TriangleAdjacency2& adjacency, const Cone* triangles, const unsigned int* live_triangles, const unsigned char* used, float meshlet_expected_radius, float cone_weight, unsigned int* out_extra)
{
	...
	for (size_t i = 0; i < meshlet.vertex_count; ++i)
	{
		...
		DEBUG("-------meshlet.vertex_count-------\n");
		DEBUG("index: %i\n", index);
		DEBUG("adjacency.counts[0]: %u\n", adjacency.counts[0]);
		DEBUG("adjacency.counts[4]: %u\n", adjacency.counts[4]);
		DEBUG("adjacency.counts[index]: %i\n", adjacency.counts[index]);
		DEBUG("neighbors_size: %lu\n", neighbors_size);
...
size_t meshopt_buildMeshlets(meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices, unsigned char* meshlet_triangles, const unsigned int* indices, size_t index_count, const float* vertex_positions, size_t vertex_count, size_t vertex_positions_stride, size_t max_vertices, size_t max_triangles, float cone_weight)
{
	...
	DEBUG("Building meshlets bro\n");

	// DEBUG("meshlets.length", meshlets.length);
	// DEBUG("meshlet_vertices", meshlet_vertices.length);
	// DEBUG("meshlet_triangles", meshlet_triangles.length);
	// DEBUG("indices", indices.length);
	// DEBUG("index_count: %i\n", index_count);
	// DEBUG("vertex_positions", vertex_positions.length);
	// DEBUG("vertex_count: %i\n", vertex_count);
	// DEBUG("vertex_positions_stride: %i\n", vertex_positions_stride);
	// DEBUG("max_vertices: %i\n", max_vertices);
	// DEBUG("max_triangles: %i\n", max_triangles);
	// DEBUG("cone_weight: %f\n", cone_weight);
	...
	DEBUG("triangle_area_avg: %f\n", triangle_area_avg);
	DEBUG("meshlet_expected_radius: %f\n", meshlet_expected_radius);
	...
	DEBUG("sizeof(Cone) / sizeof(float): %i\n", sizeof(Cone) / sizeof(float));
	...
	if (meshlet.triangle_count)
	{
		...
		DEBUG("cpp if (meshlet.triangle_count)\n");
		DEBUG("\tmeshlet_offset: %i\n", meshlet_offset);
		DEBUG("\tmeshlet.triangle_offset: %i\n", meshlet.triangle_offset);
		DEBUG("\tmeshlet.triangle_count: %i\n", meshlet.triangle_count);
		DEBUG("\tmeshlet.vertex_offset: %i\n", meshlet.vertex_offset);
		DEBUG("\tmeshlet.vertex_count: %i\n", meshlet.vertex_count);
